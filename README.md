...
- `<container>` - Контейнер в котором искать пользователей в **Active Directory**. Можно указать **GUID**, **cn**, **distinguishedName**, **sAMAccountName** или **LDAP-SQL** запрос с шаблонизацией и переменными `{select}`, `{scheme}`, `{parent}`. В качестве **LDAP-SQL** запроса можно указать часть запроса начиная с `WHERE`.
...
```
"Текст для примера|| {author.name} проекта {project.name>normal}| {done} %|."
```
...
- `normal` - Делает первый символ заглавным и удаляет точку в конце, если она едиственная. Так же удаляет `FW:` и `RE:` в начале значения.
