...
- `crop` - Удаляет блок, связанный с пересылкой письма и всё что идёт до него, а также удаляются подпись и первый символ делается заглавным. Идентификация блока пересылки идёт по последней строки в нём, начиная с `Importance:`, `Важность:`, `Subject:`, `Тема:`, `Deadline:`, `Срок:`, `From:` или `От:`.
